...
{	
	...
	public function getHeaders(){
		...
		$guest_expires_time =  session('guest_expires_time');

		$current = date('Y-m-d H:i:s');

		//if(session('guest_access_token')=='' || $current >= $guest_expires_time){
		if(1){
			...
			if($response->successful()){
				...
				session(['guest_access_token' => $response->access_token]);

				$minutes_to_add = $response->expires_in / 60;
                $time = new DateTime();
                $time->add(new DateInterval('PT' . floor($minutes_to_add) . 'M'));
                $stamp = $time->format('Y-m-d H:i:s');
                session(['guest_expires_time' => $stamp]);
				
	        	session(['timeout_guest' => 1]);
			 ...
			 $access_token= session('guest_access_token');
		}
