...
{	
	...
	public function getHeaders(){
		...
		$user_type = 'usr_type_4';
		$guest_expires_time =  session('aspirant_expires_time');

		$current = date('Y-m-d H:i:s');

		if(session('aspirant_access_token')=='' || $current >= $guest_expires_time){
			...
			if($response->successful()){
			 ...
			 $access_token= session('aspirant_access_token');
		}
