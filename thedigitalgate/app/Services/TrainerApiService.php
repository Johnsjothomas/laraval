...
{	
	...
	public function getHeaders(){
		...
		$user_type = 'usr_type_3';
		$guest_expires_time =  session('trainer_expires_time');

		$current = date('Y-m-d H:i:s');

		if(session('trainer_access_token')=='' || $current >= $guest_expires_time){
			...
			if($response->successful()){
                ...
                session(['trainer_expires_time' => $stamp]);
			 ...
			 $access_token= session('trainer_access_token');
		}
