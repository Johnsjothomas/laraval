...
class JobController extends Controller
{
    ...
    {   
        ...
        if($status=='closed'){
            $status_array = ['Closed','Deleted','Paused'];
        }

        $search_request['criteria'] = [
                    "Job_Status" => $status_array,
    ...
    {    
         ...
         if($request->isMethod('post')){
           ...
           $job_details = [
                ...
                "Mandatory_Skillset" => !empty($request->mandatory_skillset) ? implode(',',$request->mandatory_skillset) : '',
                "Total_Experience" => $request->total_experience,
                "Relevant_Experience" => $request->relevant_experience,
                "Posted_by_Company" => $request->session()->get('company_id'),
                "Gender" => $request->gender,
                "Optional_Skillset" => !empty($request->optional_skillset) ? implode(',',$request->optional_skillset):'',
           ...
           if($request->session()->get('role')=='employer'){
            ...
            if(!empty($request->jd_document)){
                ...
                $request->jd_document->move(public_path('uploads/resume'), $fileName);
    ...
    public function application_status(Request $request,$txnid,$status)
    {   
       
        $request_json = [
                "Applicant_Status" => $status
        ];
        $response = $this->hooper->updateTransaction('svc_type_9',$txnid,$request_json);
