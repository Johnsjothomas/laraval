...
<div class="container pt-5 pb-5">
	<h3 class=" text-left">Ongoing or Completed Tranings</h3>

	<table class="a table">
		<tr>
			<th class="tableborder normalfont1 pinkbackground center p1 bold">Trainings</th>
			<th class="tableborder normalfont1 pinkbackground center p1 bold">{{repalceWithMentor('Trainer')}}</th>
			<th class="tableborder normalfont1 pinkbackground center p1 bold">Acion</th>
		</tr>
		@foreach($startedtraining as $st)
		<tr>
			<td><b>{{ $st->moduleTitle }}</b></td>
			<td><b>{{ $st->first_name }} {{ $st->second_name }}</b></td>
			<td><a href="{{ route('aspirantmessagesupid',['supid'=>$st->module_Id]) }}" class="btn btn-primary btn-sm">Support</a></td>
		</tr>
		@endforeach
	</table>
</div>
                        ...
                        <div class="the_digital_gate_chatbx_main">
                            ...
                            @if (@$userArr)
                                @foreach ($userArr as $item)
                                    <div class="the_digital_gate_chatbx the_digital_gate_chatbx_js" data_id="{{$item['trainer_id']}}" data_name="{{$item['first_name'].' '.$item['second_name']}}">
                    ...
                    <div class="the_digital_gate_single_chat chat_datas">
                        <div class="chat_head">
                            ...
                            >
                                ...
                                />
                                <div>
                                    <span class="bname chat_names chat_names_js">{{repalceWithMentor('Trainer')}} name</span>
                        ...
                        <form action="javascript:;" method="post" id="client_msg_send">
                            @csrf
                            <input type="hidden" name="trainer_id" value="0">
                            <div class="chat_foot">
...
<div class="container pt-5 pb-5">
	...
	<div class="messaging">
		<div class="inbox_msg" style="display: flex;justify-content: center;">
				...
				<div class="inbox_chat">
					<div class="chat_list active_chat">
						<div class="chat_people">
							<div class="chat_img">
								<img src="{{ asset('talent/assets\img\msg_profile.png') }}" alt="sunil">
							</div>
							<div class="chat_ib">
			...
			<div class="mesgs" style="width: 80%;">
				<div class="row" ><!--style="padding-bottom: 20px !important;">-->
					<div class="col-md-3">
						<img src="{{ asset('talent/assets\img\msg_profile.png') }}" style="width: 38%;">
					</div>
					<div class="col-md-9">
						<h5>{{$trainerdetails->first_name}} {{$trainerdetails->second_name}}</h5>
						<label class="message_status">Online</label>
						<!-- <div class="sandwich_button"> -->
							<!-- <span class="bt_1"></span> <span class="bt_5"></span> <span class="bt_1"></span> -->
						<!-- </div> -->
					</div>
				</div>
				...
				<div class="type_msg">
					<div class="input_msg_write">
						...
						<form action="{{ route('aspirantmessagesupid',['supid'=>$trainerdetails->module_Id]) }}" method="POST">
						@csrf
							<input type="text" name="message" style="border: 2px solid #007bff !important;" class="write_msg" placeholder="Say something..." />
...
@endif
<script>
function getChatWithTrainer(data_id)
{
	jQuery(".loder").show();

	$.ajax({
		url: "{{route('getChatWithTrainer')}}",
		type: "POST",
		dataType: "json",
		data: {
			data_id: data_id,
		...
		success: function(response)
		{
			if(response.status)
			{
				$('#client_msg_send [name="trainer_id"]').val(data_id);
				$('.the_digital_gate_chatbox .chat_names_js').html(response.trainer.name);
				$('.the_digital_gate_chatbox .chat_image_js').attr("src",response.trainer.profile);
		...
		$.ajax({
			...
			success: function(response)
			{
				...
				if(response.status)
				{
					...
					getChatWithTrainer(response.trainer_id);
				}
				else
				{
					alert_msg(0, response.msg);
